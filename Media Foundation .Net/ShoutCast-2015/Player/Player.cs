...
class CPlayer : COMBase, IMFAsyncCallback
{
    ...
    const int WM_APP_META = WM_APP + 3;
    ...
    public string [] GetMetaNames()
    {
        if (m_pMetadata != null)
        {
            ...
            HResult hr = m_pMetadata.GetAllPropertyNames(pValue);
            MFError.ThrowExceptionForHR(hr);

            string[] sa;
            if (pValue.GetVariantType() != ConstPropVariant.VariantType.None)
                sa = pValue.GetStringArray();
            else
                sa = new string[0];
            return sa;
        }
    ...
    public HResult GetProperty(string p, PropVariant pValue)
    {
        if (m_pMetadata != null)
           return m_pMetadata.GetProperty(p, pValue);
        ...
        try
        {
            ...
            if (Succeeded(hrStatus))
            {
                ...
                switch (meType)
                {
                    ...
                    case MediaEventType.MESourceMetadataChanged:
                        OnMetadataChanged(pEvent);
                        break;
                }
            }
            else
    ...
    #region Member Variables
        ...
        protected IMFMetadata m_pMetadata;
