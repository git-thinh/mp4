...
namespace MF_BasicPlayback
{
    public partial class Form1 : Form
    {
        ...
        private void UpdateMeta()
        {
            ...
            HResult hr = g_pPlayer.GetProperty("BitRate", pValue);
            if (hr >= HResult.S_OK)
            {
                tbBitrate.Text = pValue.GetString();
            }
            else
                tbBitrate.Text = "";

            hr = g_pPlayer.GetProperty("Title", pValue);
            if (hr >= HResult.S_OK)
            {
                tbTitle.Text = pValue.GetString();
            }
            else
                tbTitle.Text = "";

            hr = g_pPlayer.GetProperty("WM/Genre", pValue);
            if (hr >= HResult.S_OK)
            {
                tbGenre.Text = pValue.GetString();
            }
            else
                tbGenre.Text = "";
        }
